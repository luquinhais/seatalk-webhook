...
SEND_URL = os.getenv("SEATALK_GROUP_SEND_URL")  # endpoint para envio a grupo
...
def seatalk_callback():
    ...
    # Verificação inicial do SeaTalk
    if data.get("event_type") == "event_verification":
        ch = data.get("event", {}).get("seatalk_challenge")
        return jsonify({"seatalk_challenge": ch}), 200

    # Aqui você pode tratar outros eventos de callback
    print("Evento recebido:", data)
    return "ok", 200

# ✅ Aceitar POST na raiz também (para evitar 405/404)
@app.post("/")
